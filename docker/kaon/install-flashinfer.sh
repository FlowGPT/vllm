#!/usr/bin/env bash
# Replace flashinfer in a vLLM image with kaon-pinned 0.6.8.post1.
set -euo pipefail
...
case "${source}" in
  ...
  local)
    ...
    if [[ ! -d "${pkg_root}/flashinfer" ]]; then
        echo "missing ${pkg_root}/flashinfer — run docker/kaon/extract-flashinfer-packages.sh first" >&2
        exit 1
    fi
    ...
    ;;
  *)
    echo "unknown FLASHINFER_SOURCE=${source} (expected pip or local)" >&2
    exit 1
    ;;
esac
